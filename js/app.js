...
window.onload = async function() {
  ...
  // 🚀 1. 樂觀渲染：如果手機有暫存設定與帳號，直接秒進系統，跳過白屏等待
  if (cachedInit && cachedUser) {
    try {
      ...
      preloadData(); // 🚀 2. 提前在背景偷偷抓取 Update 報修資料
      isCacheLoaded = true;
    } catch(e){}
  }

  // 🚀 3. 背景默默更新與驗證 (如果權限被取消，會自動登出)
  try {
    apiCall('getInitialData').then(initData => {
      safeSetStorage('xmart_init_data', JSON.stringify(initData));
      rawStoreDataHtml = initData.storeOptions; rawVendorDataHtml = initData.vendorOptions; categoryOptionsHtml = initData.categoryOptions;
      if (!isCacheLoaded) document.querySelectorAll('.category-input').forEach(el => el.innerHTML = categoryOptionsHtml);
    }).catch(()=>{});
    ...
    if (liff.isLoggedIn()) {
      ...
      if (document.getElementById('profileImage')) {
        ...
        if(lineProfile.pictureUrl) document.getElementById('profilePicWrapper').classList.remove('hidden');
        document.getElementById('loginGreeting').innerText = `Hi, ${lineProfile.displayName} 👋`;
      }

      const authRes = await apiCall('checkLineUserRegistration', { lineUserId: lineProfile.userId });
      if (authRes && authRes.isRegistered) { 
        currentUser = authRes; 
        safeSetStorage('xmart_cached_user', JSON.stringify(authRes)); 
        if (!isCacheLoaded) { applyUserToUI(authRes); preloadData(); }
      } else { 
        if (!isCacheLoaded) showLogin(); 
        else {
          // 如果背景驗證發現帳號已被總部刪除，強制清除暫存並回登入頁
          localStorage.removeItem('xmart_cached_user');
          location.reload();
        }
      }
    } else { 
      if (!liff.isInClient()) { if(!isCacheLoaded) showLogin(); } else { liff.login({ redirectUri: LIFF_URL }); } 
    }
...
function showLogin() {
  ...
  var loadingBox = document.getElementById('systemLoadingBox');
...
async function preloadData() { 
  try { 
    if(!globalPendingData) globalPendingData = await apiCall('getAllPendingIssues'); 
  } catch(e){} 
}
...
async function submitNew() {
  ...
  try {
    ...
    const res = await apiCall('submitNewIssuesText', { store: store, issues: issues, empName: currentUser.empName, empPic: (lineProfile?lineProfile.pictureUrl:null) });
    ...
    if (photoBatch.length > 0) {
      ...
      const uploadRes = await apiCall('uploadPhotosInBackground', { submissionMeta: {store:store, issues:issues, empName:currentUser.empName, empPic:(lineProfile?lineProfile.pictureUrl:null)}, photoBatchArray: photoBatch });
...
function fetchPending() {
  const store = document.getElementById('updateStoreSelect').value; 
  if(!store) { document.getElementById('saveUpdateBtn').classList.add('hidden'); return; }
  
  const l = i18n[currentLang];
  // 🚀 換掉醜醜的白框，改為美觀的轉圈動畫與文字提示
  document.getElementById('pendingContainer').innerHTML = `
    <div class="text-center p-5 rounded border border-dashed mt-3 fade-in-section" style="background: rgba(255,255,255,0.7); border-radius: var(--radius-md);">
      <div class="spinner-border text-primary mb-3" style="width: 2.2rem; height: 2.2rem;" role="status"></div>
      <h6 class="text-primary fw-bold mb-0">${l.syncingData}</h6>
    </div>`; 
    
  document.getElementById('saveUpdateBtn').classList.add('hidden');
  if (globalPendingData) renderPendingData(store, globalPendingData); 
  else { apiCall('getAllPendingIssues').then(data => { globalPendingData = data; renderPendingData(store, data); }).catch(showError); }
}
...
function renderPendingData(store, allData) {
  ...
  issues.forEach((iss, idx) => {
    ...
    let photoHtml = iss.fileId ? `
      <div class="text-center mb-3 image-loading-wrapper" style="position:relative; min-height: 160px; background: rgba(243, 244, 246, 0.7); border-radius: 8px; display:flex; align-items:center; justify-content:center; border: 1px dashed #d1d5db;">
        <div class="spinner-border text-secondary" style="position:absolute; width:1.5rem; height:1.5rem; z-index:1;" role="status"></div>
        <img src="https://drive.google.com/thumbnail?id=${iss.fileId}&sz=w800" 
             class="preview-img photo-loaded-anim" 
             onload="this.parentElement.style.minHeight='auto'; this.parentElement.style.border='none'; this.previousElementSibling.style.display='none';"
             onclick="openLightbox(this.src)" 
             onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\\'text-danger small fw-bold\\'>${l.errLoad}</div>';"
             style="max-width:100%; border-radius:8px; cursor:pointer; position:relative; z-index:2; display:block;">
